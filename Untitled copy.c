...
void create_hero(SDL_Renderer *ren, SDL_Texture *player, SDL_Texture *background, SDL_Texture *palisade, SDL_Texture *coin){
    ...
    SDL_Rect coin_Pos = {30, 30, MX_PALISADE, MX_PALISADE};
    ...
    while(!quit){
        ...
        if(player_RECT.y >= MX_SCREEN_HEIGHT - MX_TILE_SIZE - frameH) player_RECT.y = MX_SCREEN_HEIGHT - MX_TILE_SIZE - frameH;
        ...
        for(int i = 0; i <=MX_SCREEN_WIDTH; i+=MX_TILE_SIZE){
            for (int  j = 0;j <= MX_SCREEN_WIDTH; j+=MX_TILE_SIZE){
                if(i == 0||i == MX_SCREEN_WIDTH-MX_TILE_SIZE){
                    renderTexture(palisade, ren, i, j, MX_TILE_SIZE, MX_TILE_SIZE);
                }
                if(j == 0||j == MX_SCREEN_HEIGHT-MX_TILE_SIZE){
                    renderTexture(palisade, ren, i, j, MX_TILE_SIZE, MX_TILE_SIZE);
                }
            }
